 ...
 *     │
 * Ring 6 (this): ──► ALTER TABLE invoices ADD COLUMN amount <dialect-type>
 *     │
 ...
 * TYPE MAPPING
 * ============
 * Type mapping is handled by DatabaseDialect.addColumn(), which maps
 * abstract field types to dialect-specific SQL types:
 * - SQLite: TEXT, INTEGER, REAL, BLOB (type affinity)
 * - PostgreSQL: TEXT, INTEGER, NUMERIC, BOOLEAN, TIMESTAMPTZ, etc.
 *
 * INVARIANTS
 * ==========
 * INV-1: Only runs for 'create' operation on 'fields' table
 * INV-2: Silently ignores "column already exists" errors (idempotent)
 * INV-3: Uses dialect for type mapping (supports SQLite and PostgreSQL)
 *
...
export class DdlCreateField extends BaseObserver {
     ...
     * 2. Use dialect to generate ALTER TABLE ADD COLUMN
     * 3. Ignore "duplicate column" errors (idempotent)
     *
    ...
    async execute(context: ObserverContext): Promise<void> {
        ...
        const sql = system.db.dialect.addColumn(modelName, fieldName, fieldType);
        ...
        try {
            ...
            // PostgreSQL: "column X already exists"
