...
function detectType(entry: Stat, content?: Uint8Array): { type: string; mimeType: string } {
    ...
    // By extension
    if (name.endsWith('.json')) return { type: 'JSON data', mimeType: 'application/json' };
    if (name.endsWith('.xml')) return { type: 'XML document', mimeType: 'application/xml' };
    if (name.endsWith('.html') || name.endsWith('.htm')) return { type: 'HTML document', mimeType: 'text/html' };
    if (name.endsWith('.css')) return { type: 'CSS stylesheet', mimeType: 'text/css' };
    if (name.endsWith('.js')) return { type: 'JavaScript source', mimeType: 'application/javascript' };
    if (name.endsWith('.ts')) return { type: 'TypeScript source', mimeType: 'application/typescript' };
    if (name.endsWith('.md')) return { type: 'Markdown document', mimeType: 'text/markdown' };
    if (name.endsWith('.txt')) return { type: 'ASCII text', mimeType: 'text/plain' };
    if (name.endsWith('.csv')) return { type: 'CSV data', mimeType: 'text/csv' };
    if (name.endsWith('.yaml') || name.endsWith('.yml')) return { type: 'YAML document', mimeType: 'application/x-yaml' };
    if (name.endsWith('.sh')) return { type: 'shell script', mimeType: 'application/x-sh' };
    if (name.endsWith('.sql')) return { type: 'SQL script', mimeType: 'application/sql' };
    if (name.endsWith('.png')) return { type: 'PNG image', mimeType: 'image/png' };
    if (name.endsWith('.jpg') || name.endsWith('.jpeg')) return { type: 'JPEG image', mimeType: 'image/jpeg' };
    if (name.endsWith('.gif')) return { type: 'GIF image', mimeType: 'image/gif' };
    if (name.endsWith('.svg')) return { type: 'SVG image', mimeType: 'image/svg+xml' };
    if (name.endsWith('.pdf')) return { type: 'PDF document', mimeType: 'application/pdf' };
    if (name.endsWith('.zip')) return { type: 'Zip archive', mimeType: 'application/zip' };
